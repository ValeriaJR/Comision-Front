...
export default function LiderTabla({ dataMes }) {
  ...
  const totalFactura = dataMes.reduce((acum, factura) => {

    const valorLimpio = factura.resultado_factura?.replace(/[^\d.-]/g, '') || "0";
    const valorNumerico = parseFloat(valorLimpio);
  
    return acum + (isNaN(valorNumerico) ? 0 : valorNumerico);
  }, 0);
  
  const totalFacturaFormatoCOP = new Intl.NumberFormat("es-CO", {
    style: "currency",
    currency: "COP",
  }).format(totalFactura);
  
  console.log("Total de factura (COP):", totalFacturaFormatoCOP);
        ...
        <table className="min-w-full border-collapse shadow-lg">
          ...
          <tbody>
            ...
            <tr className="bg-gray-500 bg-opacity-40 px-2 py-2 text-left ">
              ...
              <th className="">{totalFacturaFormatoCOP}</th>
              <th className=""></th>
              <th className=""></th>
              <th className=""></th>
