...
export default function ButtonLider({ onChange }) {
  ...
  return <>
    ...
    >
      ...
      <select onClick={onChange}
        ...
        <option disabled>Lider</option>
        {lider.map((item, index) => (<option key={index} value={item.lider} onClick={() => onchange(item.lider)}>{item.lider}</option>))}
      </select>
